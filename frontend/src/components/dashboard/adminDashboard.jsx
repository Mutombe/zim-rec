...
const DocumentLink = ({ label, fileUrl }) => {
  if (!fileUrl) return null;

  return (
    <div className="flex items-center justify-between p-2 bg-white rounded hover:bg-gray-100">
...
const AdminDashboard = () => {
  ...
  const { loading: requestsLoading, error: requestsError } = useSelector(
    ...
    if (window.confirm("Are you sure you want to approve this device?")) {
      dispatch(
        updateDevice({
          ...
          data: {
            // Add 'data' wrapper
            status: "Approved", // Use lowercase to match backend
          },
        })
      )
        .unwrap()
  ...
  const handleDeleteDevice = (deviceId) => {
    if (
      window.confirm("Are you sure you want to permanently delete this device?")
    ) {
  ...
  const filteredDevices = allDevices
    .filter((device) => {
      ...
      const matchesStatus =
        deviceFilters.status === "" || device.status === deviceFilters.status;
      const matchesFuelType =
        deviceFilters.fuelType === "" ||
        device.device_fuel === deviceFilters.fuelType;
  ...
  const filteredRequests = allIssueRequests
    .filter((request) => {
      const matchesSearch =
        ...
        request.title
          ?.toLowerCase()
          .includes(requestFilters.search.toLowerCase()) ||
        request.description
          ?.toLowerCase()
          .includes(requestFilters.search.toLowerCase()) ||
        request.user?.name
  ...
  const uniqueFuelTypes = [
    ...new Set(allDevices.map((device) => device.device_fuel).filter(Boolean)),
  ];

  // Extract unique status values for filtering
  const uniqueDeviceStatuses = [
    ...new Set(allDevices.map((device) => device.status).filter(Boolean)),
  ];
  const uniqueRequestStatuses = [
    ...new Set(
      allIssueRequests.map((request) => request.status).filter(Boolean)
    ),
  ];
                      ...
                      onClick={() =>
                        setDeviceFilters({
                          ...
                          sortDirection:
                            ...
                            deviceFilters.sortDirection === "asc"
                              ? "desc"
                              : "asc",
                        })
                      }
 ...
 >
   <XCircle className="w-5 h-5" />
          ...
          <TabPanel value={tabValue} index={1}>
                      ...
                      onClick={() =>
                        setRequestFilters({
                          ...
                          sortDirection:
                            ...
                            requestFilters.sortDirection === "asc"
                              ? "desc"
                              : "asc",
                        })
                      }
            ...
            <div className="mt-4 text-sm text-gray-500">
              Showing {filteredRequests.length} of {allIssueRequests.length}{" "}
              issue requests
            </div>
          </TabPanel>
        </motion.div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <p className="mb-2">
                        ...
                        {selectedDevice.user?.username || "Unknown"}
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Status:</span>
                        <span
                          className={`ml-2 px-3 py-1 rounded-full text-xs font-medium ${getStatusBadgeStyle(
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <p className="mb-2">
                        ...
                        {selectedDevice.fuel_type}
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Technology:</span>{" "}
                        {selectedDevice.technology_type}
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Capacity:</span>{" "}
                        {Number(selectedDevice.capacity).toLocaleString()} MW
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Commission Date:</span>{" "}
                        {selectedDevice.commission_date
                          ? new Date(
                              selectedDevice.commissioning_date
                            ).toLocaleDateString()
                          : "N/A"}
                      </p>
                    </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg space-y-2">
                      <DocumentLink
                        label="Production Facility Registration"
                        fileUrl={
                          selectedDevice.production_facility_registration
                        }
                      />
                      <DocumentLink
                        label="Declaration of Ownership"
                        fileUrl={selectedDevice.declaration_of_ownership}
                      />
                      <DocumentLink
                        label="Metering Evidence"
                        fileUrl={selectedDevice.metering_evidence}
                      />
                      <DocumentLink
                        label="Single Line Diagram"
                        fileUrl={selectedDevice.single_line_diagram}
                      />
                      <DocumentLink
                        label="Project Photos"
                        fileUrl={selectedDevice.project_photos}
                      />
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">
                      <p className="mb-2">
                        <span className="font-medium">Device Name:</span>{" "}
                        {selectedRequest.device?.device_name || "N/A"}
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Fuel Type:</span>{" "}
                        {selectedRequest.device?.device_fuel || "N/A"}
                      </p>
                      <p className="mb-2">
                        <span className="font-medium">Capacity:</span>{" "}
                        {selectedRequest.device?.capacity
                          ? Number(
                              selectedRequest.device.capacity
                            ).toLocaleString() + " MW"
                          : "N/A"}
