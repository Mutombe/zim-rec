...
export const AuthModals = ({ openType, onClose }) => {
  ...
  const handleSubmit = () => {
    if (view === "login") {
      dispatch(login({ username: formData.username, password: formData.password }))
        .unwrap()
        .then(() => { toast.success("You are logged in!"); onClose(); })
        .catch((err) => console.error("Login Failed:", err));
    } else {
      dispatch(register(formData))
        .unwrap()
        .then(() => { toast.success("Registration successful. You may Sign-In"); onClose(); })
  ...
  const handleGoogleSuccess = (credentialResponse) => {
    dispatch(googleLogin({ credential: credentialResponse.credential }))
      .unwrap()
      .then(() => { toast.success("Signed in with Google!"); onClose(); })
  ...
  const inputClass =
    "w-full pl-10 pr-4 py-3 border border-gray-300 rounded-xl text-sm text-gray-900 placeholder-gray-400 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500 transition-all";
  ...
  return (
    <>
                  ...
                  <div className="mt-auto border-t border-gray-100 pt-5">
                    {isAuthenticated ? (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 border border-red-200 text-red-600 hover:bg-red-50 rounded-xl py-2.5 font-medium transition-colors"
                      >
                        <LogOut size={18} />
                        Sign Out
                      </button>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl py-2.5 font-medium shadow-sm transition-colors"
                        >
                          <UserPlus size={16} />
                          Create Account
                        </button>
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-2 border border-gray-300 text-gray-700 hover:border-emerald-500 rounded-xl py-2.5 font-medium transition-colors"
                        >
                          <LogIn size={16} />
                          Sign In
                        </button>
      ...
      >
        ...
        <MenuItem
          onClick={() => { dispatch(logout()); handleUserMenuClose(); }}
